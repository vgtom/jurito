...
import { type PrismaClient, UserPlan } from "@prisma/client";
import crypto from "crypto";
import express from "express";
import { HttpError, type MiddlewareConfigFn } from "wasp/server";
import { type PaymentsWebhook } from "wasp/server/api";
import { userPlanForActiveSubscription } from "../../server/billing/planFromPayment";
...
async function handleOrderCreated(
  ...
  let planUpdate: UserPlan | undefined = undefined;
  if (status === "paid" && plan.effect.kind === "credits") {
    numOfCreditsPurchased = plan.effect.amount;
    datePaid = new Date();
  }
  if (status === "paid" && plan.effect.kind === "subscription") {
    datePaid = new Date();
    planUpdate = userPlanForActiveSubscription(planId);
  }
  ...
  await updateUserLemonSqueezyPaymentDetails(
    {
      ...
      datePaid,
      ...(planUpdate !== undefined ? { plan: planUpdate } : {}),
    },
    prismaUserDelegate,
  );
    ...
    await updateUserLemonSqueezyPaymentDetails(
      {
        ...
        datePaid: new Date(),
        plan: userPlanForActiveSubscription(planId),
      },
      prismaUserDelegate,
    );
  } else {
